...
from Mariyan import YouTube, app
from Mariyan.core.call import Vishnu
from Mariyan.misc import SUDOERS, db
from Mariyan.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from Mariyan.utils.decorators.language import languageCB
from Mariyan.utils.formatters import seconds_to_min
from Mariyan.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from Mariyan.utils.stream.autoclear import auto_clean
from Mariyan.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Vishnu.pause_stream(chat_id)
        ...
        await Vishnu.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Vishnu.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Vishnu.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Vishnu.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Vishnu.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
